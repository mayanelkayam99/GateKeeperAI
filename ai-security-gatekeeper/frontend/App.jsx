          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 12, color: C.textMuted, marginTop: 2 }}>
                This will override the block on{" "}
                <code style={{ fontFamily: C.mono, background: C.surface2, padding: "1px 5px", borderRadius: 4 }}>
        ...
        <div style={{ padding: "20px 24px" }}>
          ...
          <div style={{ background: C.redDim, border: `1px solid ${C.redBorder}`, borderRadius: 10, padding: "12px 16px", marginBottom: 20 }}>
            ...
            <div style={{ fontSize: 12, color: C.textMuted, lineHeight: 1.6 }}>
              You are bypassing an AI-enforced security block. This action is logged and auditable.
              Only proceed if you have assessed and accepted the associated risk.
            </div>
          </div>
          ...
          <div style={{ marginBottom: 20 }}>
            <label style={{ display: "block", fontSize: 12, fontWeight: 700, color: C.text, marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.06em" }}>
              Reason for Override <span style={{ color: C.red }}>*</span>
            </label>
            <textarea
              ref={inputRef}
              value={reason}
              onChange={(e) => setReason(e.target.value)}
              placeholder="e.g. Vulnerability is in a dev-only path, confirmed with security team, patching in next sprint..."
              rows={4}
              style={{
                width: "100%", padding: "10px 12px", borderRadius: 8, fontSize: 13,
                fontFamily: C.sans, border: `1.5px solid ${reason.trim().length >= 10 ? C.amberBorder : C.border}`,
                outline: "none", resize: "vertical", background: C.bg, color: C.text,
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
            <button onClick={onCancel} disabled={loading} style={{
              padding: "9px 20px", borderRadius: 8, border: `1.5px solid ${C.border2}`,
              background: "transparent", fontSize: 13, fontWeight: 600, color: C.textMuted,
              cursor: "pointer",
            }}>
              Cancel
            </button>
            <button
              ...
              style={{
                ...
                background: canSubmit ? C.amber : C.textDim,
...
export default function App() {
  ...
  // Main tab (viewer vs policy)
  const [activeMainTab, setActiveMainTab] = useState("viewer");
  ...
  const handleOverrideConfirm = async (reason) => {
    ...
    try {
      ...
      // Refresh sidebar history to reflect new status
      await loadHistory();
  ...
  const canOverride = isBlocked || isWarning;
  ...
  return (
    <div style={{ fontFamily: C.sans, minHeight: "100vh", background: C.bg, color: C.text }}>
      ...
      <header style={{ background: C.surface, borderBottom: `1px solid ${C.border}`, padding: "0 2rem", display: "flex", alignItems: "center", justifyContent: "space-between", height: 60 }}>
        ...
        {result && (
          <div style={{
            ...
            borderRadius: 999, fontWeight: 600, fontSize: 12, letterSpacing: "0.04em",
      ...
      {/* ── TAB BAR ── */}
      <div style={{ background: C.surface, borderBottom: `1px solid ${C.border}`, padding: "0 2rem", display: "flex", gap: 4 }}>
        {[
          { id: "viewer", label: "🔍 Dependency Auditor" },
          { id: "policy", label: "🛡️ CISO AI Governance" },
        ].map((t) => (
          <button key={t.id} onClick={() => setActiveMainTab(t.id)} style={{
            padding: "10px 18px", border: "none", background: "transparent", cursor: "pointer",
            fontSize: 13, fontWeight: 600,
            color: activeMainTab === t.id ? C.orange : C.textMuted,
            borderBottom: activeMainTab === t.id ? `2.5px solid ${C.orange}` : "2.5px solid transparent",
        ...
        <div style={{ display: "flex", height: "calc(100vh - 96px)" }}>
          ...
          <main style={{ flex: 1, overflowY: "auto", padding: "24px 28px", background: C.bg }}>
            ...
            <div style={{ background: C.surface, borderRadius: 12, padding: "18px 22px", border: `1px solid ${C.border}`, marginBottom: 20, boxShadow: "0 1px 3px rgba(0,0,0,0.05)" }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: C.textMuted, marginBottom: 14, display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <div style={{ padding: "20px 24px", background: sc.bg, borderBottom: `1px solid ${sc.border}` }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12 }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 10 }}>
                      ...
                      {canOverride && (
                        <button
                          onClick={() => { setOverrideError(null); setShowOverrideModal(true); }}
                          style={{
                            padding: "6px 14px", borderRadius: 8, fontSize: 12, fontWeight: 600,
                            background: "transparent", color: C.amber,
                            border: `1.5px solid ${C.amberBorder}`, cursor: "pointer",
                  ...
                  <div style={{ marginTop: 10, fontSize: 13, color: C.textMuted }}>
                    ...
                    {isBlocked
                      ? result.ai_explanation?.toLowerCase().includes("rce") ? "Blocked — critical RCE risk detected."
                        : result.ai_explanation?.toLowerCase().includes("prototype") ? "Blocked — prototype pollution risk detected."
                          : "Blocked — security policy violated."
                      : isOverridden ? "Override accepted — developer acknowledged risk and approved push."
                        : isWarning ? "Warning — requires manual review before release."
                          : "Approved — no blocking violations found."}
                  </div>

                  {/* Override error */}
                  {overrideError && (
                    <div style={{ marginTop: 10, fontSize: 12, color: C.red, background: C.redDim, padding: "8px 12px", borderRadius: 6, border: `1px solid ${C.redBorder}` }}>
                      ⚠️ {overrideError}
                    </div>
                  )}
                </div>
                ...
                <div style={{ padding: "22px 24px" }}>
                      ...
                      <tbody>
                        <tr>
                          ...
                          <td style={{ padding: "12px 14px" }}>
                            <span style={{ padding: "3px 10px", borderRadius: 999, fontSize: 11, fontWeight: 700, background: sc.bg, color: sc.color, border: `1px solid ${sc.border}` }}>
                              {status}
                            </span>
                          </td>
                  ...
                  {activeTab === "fix" && (
                    <div style={{ background: C.greenDim, border: `1.5px solid ${C.greenBorder}`, borderRadius: 12, padding: "18px 22px" }}>
                      <div style={{ fontSize: 15, fontWeight: 700, color: C.green, marginBottom: 10, display: "flex", alignItems: "center", gap: 8 }}>
                        🔧 Recommended Fix
                      </div>
                      <div style={{ fontSize: 14, lineHeight: 1.8, color: C.text, whiteSpace: "pre-wrap" }}>
                        {result.recommendation || "Run `npm audit fix` to apply patches, or search npmjs.com for a secure alternative."}
                      </div>
                    </div>
                  )}
                    ...
                    <div>
                      ...
                      <div style={{ marginTop: 18, background: "#0d1117", borderRadius: 10, padding: "16px 20px", fontFamily: C.mono, fontSize: 12, lineHeight: 2, border: "1px solid #30363d" }}>
                        ...
                        {[
                          ...
                          result.cvss_max_score >= 7
                            ? `[ALERT]   CVSS ${result.cvss_max_score?.toFixed(1)} exceeds threshold 7.0 → escalating to BLOCK`
                            : `[CHECK]   CVSS score within acceptable range`,
                          `[LEGAL]   Running SPDX license compliance check (${result.license_type || "Unknown"})…`,
                          `[AI]      Groq Llama 3.3-70B generating risk explanation…`,
                          isOverridden
                            ? `[OVERRIDE] Developer accepted risk — status set to OVERRIDDEN`
                            : `[VERDICT] Final status: ${status} — push ${isBlocked ? "rejected" : "allowed"}`,
                        ].map((line, i) => (
                    ...
                    <div style={{ display: "flex", flexDirection: "column", height: 420 }}>
                      ...
                      <div style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column", gap: 10, marginBottom: 12 }}>
                        ...
                        {chatMessages.map((m, i) => (
                          <div key={i} style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start" }}>
                            <div style={{
                              maxWidth: "85%", borderRadius: 12, padding: "10px 14px", fontSize: 13, lineHeight: 1.7, whiteSpace: "pre-wrap",
                              background: m.role === "user" ? C.orange : C.surface2,
                      ...
                      <div style={{ display: "flex", gap: 10 }}>
                        ...
                        <button onClick={() => sendChat()} disabled={chatStreaming || !chatInput.trim()} style={{
                          padding: "9px 18px", borderRadius: 8, background: C.orange,
